...
void AST::swap(AST& b) {
	...
	//swaps member v (node, text tag)

	nodes tmpNode; //swap node
     tmpNode = nodeType;
     nodeType = b.nodeType;
     b.nodeType = tmpNode;
    
    
     std::string tmp; //swap text
     tmp = text;
     text = b.text;
     b.text = tmp;
     ...
     tmp = closeTag; //swap close tag
     closeTag = b.closeTag;
     b.closeTag = tmp;
    
     child.swap(b.child); //swap to swap child
}
...
void AST::mainHeader(const std::vector<std::string>& profileName) {
	...
	std::string profileS = "#include \"profile.hpp\"";
   	std::list<AST*>::iterator obj = child.begin();
    	...
    	child.insert(obj, new AST(token, profileS));

    	std::vector<std::string>::const_iterator it = profileName.begin();

    	while(it != profileName.end()) {     
        
     	std::string fileNCpp = (*it); 	//og profile name

      	size_t i = 0; 
      	while(i != fileNCpp.length()){
          	if(fileNCpp[i] == '_')
               	fileNCpp[i] = '.';		//replaces _ with .
            			++i;           
        	} 
...
void AST::fileHeader(const std::string& profileName) {
	...
	std::list<AST*>::iterator obj = child.begin();
    	...
    	std::string fileNCpp = profileName;
    	size_t i = 0; 
